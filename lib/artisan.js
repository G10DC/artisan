...
export function transformToPedagogicalHtml(markdownText, options = {}) {
  const { title = 'Manuale di Studio', courseCode = '', theme = 'dark', enableOralMode = true, enableTTS = true } = options;
  ...
  processedHtml = injectExamFrequencyBadges(processedHtml);
...
<body>
  <header style="padding: 1rem 2rem; background: var(--bg-surface); border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
    ...
    <div style="display: flex; gap: 0.8rem; align-items: center;">
      ...
      <button onclick="document.body.classList.toggle('dyslexic-font')" style="background: var(--bg-body); color: var(--text-main); border: 1px solid var(--border-color); padding: 0.3rem 0.7rem; border-radius: 12px; cursor: pointer; font-size: 0.78rem;">🔤 Font DSA</button>
    </div>
  </header>
